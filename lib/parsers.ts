...
export function parseExplanationJson(rawData: any): DeepAnalysis {
    ...
    const parsedResearch = rawData?.research ? {
        confidence: typeof rawData.research.confidence === 'number' ? rawData.research.confidence : undefined,
        unknowns: Array.isArray(rawData.research.unknowns)
            ? rawData.research.unknowns.map((u: any) => String(u))
            : undefined,
        sources: Array.isArray(rawData.research.sources)
            ? rawData.research.sources
                .filter((s: any) => s && (s.claim || s.reference))
                .map((s: any) => ({
                    claim: String(s.claim || ""),
                    source_type: String(s.source_type || "secondary"),
                    reference: String(s.reference || ""),
                    reliability: String(s.reliability || "medium"),
                    url: typeof s.url === 'string' ? s.url : undefined
                }))
            : undefined
    } : undefined;

    const parsedCritique = rawData?.victor_critique ? {
        strengths: Array.isArray(rawData.victor_critique.strengths)
            ? rawData.victor_critique.strengths.map((s: any) => String(s))
            : undefined,
        gaps: Array.isArray(rawData.victor_critique.gaps)
            ? rawData.victor_critique.gaps.map((g: any) => String(g))
            : undefined,
        next_pass_focus: typeof rawData.victor_critique.next_pass_focus === 'string'
            ? rawData.victor_critique.next_pass_focus
            : undefined
    } : undefined;

    // 0. PASS-THROUGH FOR EXISTING DEEP OBJECTS
    // Supports local deep analysis files shaped like:
    // { isDeep, meta, gan, nodes, research?, victor_critique? }
    if (Array.isArray(rawData.nodes)) {
        return {
            isDeep: rawData.isDeep ?? true,
            meta: rawData.meta,
            gan: rawData.gan,
            research: parsedResearch,
            victor_critique: parsedCritique,
            nodes: rawData.nodes.map((item: any) => ({
                lyric: item.lyric || "",
                surface: item.surface || "",
                deep: Array.isArray(item.deep) ? item.deep.map((d: any) => ({
                    category: String(d.category || "THEME"),
                    text: String(d.text || "")
                })) : [],
                rhymes: Array.isArray(item.rhymes)
                    ? item.rhymes
                        .filter((r: any) => r && typeof r.word === 'string')
                        .map((r: any) => ({
                            word: r.word,
                            score: Number(r.score ?? 0),
                            phonetic_group: typeof r.phonetic_group === 'string' ? r.phonetic_group : undefined,
