...
import { getPairingForTrack } from '../lib/album-pairings';
...
export const StoryArchive: React.FC = () => {
  ...
  const pairing = getPairingForTrack(viewingArchiveId);
        ...
        >
            ...
            {activeTab === 'track' ? (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
                    {pairing && (
                        <div className="bg-white/5 border border-white/10 rounded-lg p-5 md:p-6 space-y-3">
                            <div className="flex items-center justify-between gap-3">
                                <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-amber-glow">Pairing Map</span>
                                <span className="text-[10px] uppercase tracking-widest text-white/40">
                                    Stage {pairing.stage_index}/2 ({pairing.stage_label})
                                </span>
                            </div>
                            <p className="text-sm text-white/85 font-semibold">{pairing.pair_label}</p>
                            <p className="text-xs text-white/60 leading-relaxed">
                                <strong className="text-white/80">Axis:</strong> {pairing.relationship_axis}
                            </p>
                            <p className="text-xs text-white/60 leading-relaxed">
                                <strong className="text-white/80">Partner:</strong> {pairing.partner_track_title}
                                {!pairing.partner_available && ' (pending)'}
                            </p>
                            <p className="text-xs text-white/60 leading-relaxed">
                                <strong className="text-white/80">Thread:</strong> {pairing.connective_thread}
                            </p>
                            <p className="text-xs text-white/70 italic">{pairing.short_reminder}</p>
                        </div>
                    )}
